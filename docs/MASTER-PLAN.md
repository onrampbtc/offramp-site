...
```
**Three-property architecture (decided July 2026, name locked = Offramp):**
1. offrampgold.com — the brand, the funnel, all authority.
2. goldvsbitcoin.org — Lane 2 editorial front door (merits/differences for buy/sell deciders); conversions → main site.
3. goldbuyer.io — Phase 2 (month 4+): proofofcustody-style ranking site scoring all online gold buyers against a published methodology, **with FTC-compliant footer disclosure** ("operated by the team behind Offramp"). Targets "best place to sell gold" 3,600/mo + "near me" 2,400/mo.
4. goldtobtc.com — 301 → /gold-to-bitcoin.
"Cash out" = product decision-moment verb only (vault sell CTA, alerts, offer acceptance, Lane 2 campaigns) — never company branding, never first-contact copy.
