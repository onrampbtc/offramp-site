...
**Step 4 — Keep it (the capture moment, value-for-value).** "Email me this breakdown" + market-notes checkbox. ● SHIPPED. Then the upgrade: **"Track these pieces"** → creates their ledger — V1 = email magic link (Supabase Auth OTP), V2 = Google one-tap once OAuth creds exist. Signed-in users get: pieces saved, live re-valuation, price alerts, share-with-family link. Login is NEVER required before the number — it buys tracking, not the answer. ▢ BUILD (needs: Supabase Auth config + Resend SMTP for magic links; Google OAuth client from Michael for V2)
